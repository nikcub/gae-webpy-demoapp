...
# coding: utf-8
def index (items, user, users):
    __lineoffset__ = -4
    loop = ForLoop()
    self = TemplateResult(); extend_ = self.extend
    self['title'] = join_(u'Home')
    if user:
        self['user'] = join_(escape_(user.nickname(), True))
    if users:
        self['userlogout'] = join_(escape_(users.create_logout_url("/"), True))
        self['userlogin'] = join_(escape_(users.create_login_url("/"), True))
    self['admin'] = join_(u'True')
    extend_([u'\n'])
